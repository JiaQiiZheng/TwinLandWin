...
namespace TwinLand
{
    public class Options : TwinLandComponent
    {
        /// <summary>
        ...
        {
            pManager.AddNumberParameter("Duration Time", "Duration Time", "Duration time per iteration of the Solver", GH_ParamAccess.item, defaultDt);
            pManager.AddIntegerParameter("Sub Steps", "Sub Steps", "Steps per duration time, collision detection will be excecute per sub step", GH_ParamAccess.item, 3);
            pManager.AddIntegerParameter("Iteration", "Iteration", "Iteration of per sub-step", GH_ParamAccess.item, 3);
            pManager.AddIntegerParameter("Scene Mode", "Scene Mode", "The interative mode options:\n 0 = not allow adding new particles while running.\n 1 = Append new particles whenever the particles collection changed.\n 3 = Lock up the simulation and not consider any change in scene", GH_ParamAccess.item, 0);
